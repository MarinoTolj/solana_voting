...
export function votingCard() {
  ...
  // Derive voting PDA from generated IDL client
  useEffect(() => {
    let cancelled = false;
    ...
    async function derivevoting() {
      ...
      try {
        ...
        if (!cancelled) setvotingAddress((pda as Address) ?? null);
      } catch {
        if (!cancelled) setvotingAddress(null);
      }
    }
  ...
  const votingLamports = votingBalance?.lamports;

  const handleDeposit = useCallback(async () => {
    if (!walletAddress || !votingAddress || !amount || !signer) return;
  ...
  }, [walletAddress, votingAddress, amount, signer, send, getExplorerUrl]);

  const handleWithdraw = useCallback(async () => {
    if (!walletAddress || !votingAddress || !signer) return;
  ...
  }, [walletAddress, votingAddress, signer, send, getExplorerUrl]);
    ...
    <section className="w-full space-y-4 rounded-2xl border border-border-low bg-card p-6 shadow-[0_20px_80px_-50px_rgba(0,0,0,0.35)]">
      <div className="flex items-start justify-between gap-4">
        ...
        <span className="rounded-full bg-cream px-3 py-1 text-xs font-semibold uppercase tracking-wide text-foreground/80">
          {(votingLamports ?? 0n) > 0n ? "Has funds" : "Empty"}
        </span>
      </div>
      ...
      <div className="rounded-xl border border-border-low bg-cream/30 p-4">
        ...
        <p className="mt-1 text-3xl font-bold tabular-nums">
          {votingLamports ? lamportsToSolString(votingLamports) : "0"}{" "}
          <span className="text-lg font-normal text-muted">SOL</span>
        </p>
        {votingAddress && (votingLamports ?? 0n) > 0n && (
          <p className="group mt-2 flex items-center gap-1.5">
            <a
              href={getExplorerUrl(`/address/${votingAddress}`)}
              target="_blank"
              rel="noopener noreferrer"
            ...
            <span
              ...
              title="This is your voting PDA — a program-derived account that holds your deposited SOL. Only you can withdraw from it."
            >
              <svg
      ...
      <button
        ...
        disabled={isSending || !votingLamports}
