...
export default function InitPoll() {
  ...
  function validate() {
    ...
    if (candidates.length < 2){
      setOptionsError(`You must provide at least 2 options. Current: ${candidates.length}`);
      isValid = false;
    } else {
  ...
  const handleAddCandidate = () => {
    ...
    if (candidates.some(c => c.toLowerCase() === trimmedInput.toLowerCase())) {
  ...
  const handleInitPoll = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await handleInsertPoll(pollId, pollPda, wallet.signer.address, pollName, pollDesc);
      ...
      <form onSubmit={handleInitPoll} className="space-y-6">
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-4 py-2 rounded-lg border border-input bg-background text-foreground placeholder:text-foreground/40 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition"
          />
          {descError && <p className="mt-1.5 text-sm text-destructive">{descError}</p>}
        </div>
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-4 py-2 rounded-lg border border-input bg-background text-foreground placeholder:text-foreground/40 focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition"
          />
          {durationError && <p className="mt-1.5 text-sm text-destructive">{durationError}</p>}
        </div>
        ...
        <div>
          ...
          {optionsError && <p className="mt-1.5 text-sm text-destructive">{optionsError}</p>}
        </div>

        {candidates.length > 0 && (
          <div>
            <label className="block text-sm font-medium mb-2">Options ({candidates.length})</label>
            <div className="space-y-2">
              {candidates.map((candidate, index) => (
                <div key={index} className="flex items-center justify-between px-4 py-3 rounded-lg bg-card border border-border">
                  <span className="text-sm">{index + 1}. {candidate}</span>
                  <button
                    type="button"
